...
mort_plot <- elings_data %>%
  ...
  summarise(
    total_plants = n(),
    alive_plants = sum(mort_status),
    alive_prop = mean(mort_status),
    alive_pct = alive_prop * 100,
    .groups = "drop")

ggplot(mort_plot,
       aes(x = plot_id, y = alive_pct, fill = treatment)) +
  geom_col() +
  ylim(0, 100) +
  theme_bw() +
  labs(x = "Plot ID",
...
# adding error bars
plot_survival <- elings_data %>%
  group_by(plot_id, treatment) %>%
  summarise(
    alive_pct = 100 * mean(mort_status),
    .groups = "drop")


treatment_survival <- plot_survival %>%
  group_by(treatment) %>%
  summarise(
    mean_alive = mean(alive_pct),
    sd = sd(alive_pct),
    n = n(),
    se = sd / sqrt(n),
    .groups = "drop")
...
ggplot(treatment_survival,
  ...
  geom_errorbar(
    aes(
      ymin = mean_alive - se,
      ymax = mean_alive + se), width = 0.2) +
  theme_bw() +
  labs(x = "Treatment", y = "Mean Plants Alive (%)")
